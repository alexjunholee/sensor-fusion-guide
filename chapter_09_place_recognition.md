...
BoW의 한계는 세 가지다. 양자화 과정에서 정보 손실이 크고(비슷한 특징이 같은 단어로 뭉침), 조명·시점 변화에 대한 불변성이 기저 특징(SIFT, ORB)에 의존하며, vocabulary 크기 $K$ 선택이 성능에 큰 영향을 미친다.
...
방법은 세 단계다. 각 이미지를 극도로 간소화된 표현(저해상도 패치 또는 단순 디스크립터)으로 변환하고, 쿼리 시퀀스와 데이터베이스 시퀀스의 **시퀀스 유사도 행렬(sequence similarity matrix)**을 구성한다. 그 위에서 일정 속도 범위 내의 대각선 경로를 탐색하여 최적 시퀀스 매칭을 찾는다.
...
아키텍처는 3단계로 구성된다. Range image를 lightweight CNN으로 처리하여 feature map을 추출하고, NetVLAD 레이어로 글로벌 디스크립터를 생성하며, Transformer encoder로 전체적인 context를 반영한다. 3D 포인트 클라우드 처리보다 2D CNN 처리가 빠르고, 기존 이미지 네트워크 아키텍처를 그대로 활용할 수 있다는 점이 이 방식의 실용적 장점이다.
